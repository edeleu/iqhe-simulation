...
def computeChernGrid_gpu(thetaResolution, num_states, V):
    eig_cache = EigenvectorCache(max_bytes=16 * 1024**3, num_states=num_states)
    accumulator = cp.zeros(num_states, dtype=cp.float64)  # Keep accumulation on GPU
        ...
        for indexTWO in range(thetaResolution - 1):
            ...
            for key in keys:
                ...
                if cached_eigv is None:
                    ...
                    H_gpu = cp.asarray(H_cpu)  # Transfer to GPU
                    _, eigv_gpu = cp.linalg.eigh(H_gpu)  # Compute eigenvectors on GPU
                    eig_cache.add(key, eigv_gpu)  # Store in cache
                else:
                    eigv_gpu = cached_eigv
                eigVecs_gpu.append(eigv_gpu)
            
            # Compute Berry phase on GPU
            innerProductOne = cp.sum(cp.conj(eigVecs_gpu[0]) * eigVecs_gpu[1], axis=0)
            innerProductTwo = cp.sum(cp.conj(eigVecs_gpu[1]) * eigVecs_gpu[2], axis=0)
            innerProductThree = cp.sum(cp.conj(eigVecs_gpu[2]) * eigVecs_gpu[3], axis=0)
            innerProductFour = cp.sum(cp.conj(eigVecs_gpu[3]) * eigVecs_gpu[0], axis=0)
            
            berryPhase = cp.angle(innerProductOne * innerProductTwo * innerProductThree * innerProductFour)
            accumulator += berryPhase

    return cp.asnumpy((accumulator / (2 * cp.pi)) + 1 / num_states)

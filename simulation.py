...
def fullSimulationGrid(thetaResolution=10,visualize=False):
    ...
    eigValueGrid = np.zeros((thetaResolution,thetaResolution),dtype=object)
    ...
    for i in range(NUM_STATES):
        chernNum = computeChernGridV2(i,eigValueGrid,delTheta,thetaResolution)
        # print(chernNum)
...
def computeChernGridV2(stateNumber,grid,delTheta,thetaNUM):
    ...
    for indexONE in range(thetaNUM-1):
        for indexTWO in range(thetaNUM-1):
            ...
            # simple implementation, do this and throw away resulting imag part

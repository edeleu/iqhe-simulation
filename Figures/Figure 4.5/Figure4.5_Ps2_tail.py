...
OUT_DIR = Path("Figure 4.5"); OUT_DIR.mkdir(exist_ok=True)
...
def make_tail_Ps_figure() -> None:
    ...
    # PCTILE BASED METHOD
    lower, upper = np.percentile(s_tilde, [0, 99.])
    ...
    ax.plot(s_grid, 4*s_grid * np.exp(-2 * s_grid), color="red", lw=1.2,alpha=0.8,
            label=r"$P_2(\tilde{s})=4\tilde{s}\exp{(-2\tilde{s})}$")

    # stats: KS, p, chi²_r
    cdf_exp = lambda x: np.exp(-2 * x)*(np.exp(2*x)-2*x-1)
    ...
    def _bin_prob(a, b):
        return integrate.quad(lambda x: 4*x*np.exp(-2 * x), a, b,
                              epsabs=1e-9, epsrel=1e-9)[0]
    
    exp_probs = np.array([_bin_prob(a, b)
    ...
    fig.savefig(OUT_DIR / "tail_Ps2_N1024.pdf")
    plt.close(fig)

...
def add_quantile_trend(E: np.ndarray, r: np.ndarray, ax: Axes, cfilter,
    ...
    ax.plot(centers, r_mean, color=color, lw=1.1, marker=".",
        ...
        label = rf"$\langle\tilde{{r}}^{{(1)}}\rangle$"
        ) #label="Quantile Mean"
    if cfilter==0:
        ax.legend(loc="upper center", fontsize=6, ncols=3, frameon=False,labelcolor="#FFFFFF")
...
def hexbin_panel(E: np.ndarray, r: np.ndarray, ax: Axes, cfilter,

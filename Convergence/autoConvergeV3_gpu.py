...
NUM_STATES = 512     # Number of states for the system
...
def helper(thetaRes, V):
    ...
    return chern_numbers, chern_sum, eigspacing


def run_convergence_trials(max_resolution=50, min_resolution=5, step=2, num_trials=10000, save_path="chern_512_convergenceV3_results.csv"):
    """
    ...
    for trial in range(1, num_trials + 1):
        ...
        min_eigenvalue_spacing_Before_converged = None
        min_eigenvalue_spacing_At_converged = []
        reference_chern = None
        lastRecordedError = "None"

        # _, _, minSpacingAt30 = helper(30,V)
        minSpacingAt30 = 0

        maxThetaResolution = max_resolution
        chern_numbers, chern_sum, eigspacing = helper(maxThetaResolution,V)
        if chern_sum == 1.0:
            reference_chern = chern_numbers
            min_eigenvalue_spacing_reference = eigspacing
        ...
        slidingWindow=[]
        for thetaResolution in range(min_resolution, maxThetaResolution):
            print(f"Searching resolution {thetaResolution}...")
            chern_numbers, chern_sum, eigspacing = helper(thetaResolution,V)

            if np.allclose(chern_numbers, reference_chern):
                slidingWindow.append(thetaResolution)
                min_eigenvalue_spacing_At_converged.append(eigspacing)
            else:
                slidingWindow = []
                min_eigenvalue_spacing_At_converged = []
                min_eigenvalue_spacing_Before_converged = eigspacing
                if chern_sum != 1.0:
                    print(f"Warning: Chern numbers sum to {chern_sum} at resolution {thetaResolution}.")
                    mismatches = highlight_mismatches(reference_chern, chern_numbers)
                    print(f"Mismatch Details: {mismatches}")
                    lastRecordedError = f"SUM={chern_sum}"  
                else: # MISMATCH
                    print(f"Warning: Mismatch detected at resolution {thetaResolution}.")
                    mismatches = highlight_mismatches(reference_chern, chern_numbers)
                    print(f"Mismatch Details: {mismatches}")
                    lastRecordedError = "Mismatch"
            
            if len(slidingWindow)==5:
                print("Sliding window converged at ", slidingWindow[0], "to", slidingWindow[-1])
                break

        if len(slidingWindow)==5:
            save_to_csv(trial,slidingWindow[0],min_eigenvalue_spacing_Before_converged, min_eigenvalue_spacing_At_converged[0], min_eigenvalue_spacing_reference,lastRecordedError,minSpacingAt30,save_path)
            print(f"Trial {trial} results saved to {save_path}")
        else: 
            print(f"Trial {trial} has not converged. . .")
...
def save_to_csv(trial_number, converged_resolution, min_eigenvalue_spacing_Before_converged, min_eigenvalue_spacing_At_converged, min_eigenvalue_spacing_reference,errorType,minSpacingAt30, filename="chern_data.csv"):
    ...
    data = {
        ...
        "Min Eigenvalue at Res. Before Convergence": [min_eigenvalue_spacing_Before_converged],
        "Min Eigenvalue at Convergence": [min_eigenvalue_spacing_At_converged],
        "Min Eigenvalue Spacing Reference": [min_eigenvalue_spacing_reference],
        "Min Eigenvalue Spacing at 30x30": [minSpacingAt30],
        "Last Recorded Error": [errorType],
    }
    ...
    print(f"Data saved to {filename}. Trial {trial_number}: Resolution {converged_resolution}, Min Eigenvalue Reference {min_eigenvalue_spacing_reference},Min Eigenvalue Converged {min_eigenvalue_spacing_At_converged}")

def save_potential_matrix(V, trial_number):
    """Saves the Potential matrix V to a file when a mismatch occurs."""
    # filename = f"/scratch/gpfs/ed5754/iqheFiles/Potentials/potential_matrix_trial_{trial_number}_Nstates_{NUM_STATES}.npy"
    slurm_task_id = os.getenv("SLURM_PROCID", "unknown")  # Get SLURM task ID
    filename = f"/scratch/gpfs/ed5754/iqheFiles/Potentials/potential_matrix_trial_{trial_number}_Nstates_{NUM_STATES}_task_{slurm_task_id}.npy"
...
if __name__ == "__main__":
    ...
    run_convergence_trials(max_resolution=80, min_resolution=15)

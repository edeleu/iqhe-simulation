...
def folded_pdf(r, beta):
    r = np.asarray(r)
    return wigner_dyson_pdf(r, beta) + wigner_dyson_pdf(1/r, beta) / r**2
...
def folded_model_cdf(r_grid, beta):
    pdf_vals = folded_pdf(r_grid, beta)
...
def perform_ks_test(r_vals, beta):
    ...
    model_cdf = folded_model_cdf(r_grid, beta)
...
def qq_plot(ax, data, beta):
    ...
    cdf_vals = folded_model_cdf(r_grid, beta)
...
def main_fitr(folder_path, overlay_n_values,energy_window):
    ...
    for subdir in subdirs:
        ...
        pdf_fit = folded_pdf(bin_centers, beta_fit)

...
def nearest_chern_stats(eigs, cherns):
    ...
    for i in range(n):
        ...
        # Check left neighbor
        if i - 1 >= 0:
            left_dist = sorted_eigs[i] - sorted_eigs[i - 1]
            nearest_distance = left_dist
            nearest_chern = sorted_cherns[i - 1]

        # Check right neighbor
        if i + 1 < len(sorted_eigs):
            right_dist = sorted_eigs[i + 1] - sorted_eigs[i]
            if right_dist < nearest_distance:
                nearest_distance = right_dist
                nearest_chern = sorted_cherns[i + 1]
